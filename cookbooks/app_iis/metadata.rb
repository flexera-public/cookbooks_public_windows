...
recipe "app_iis::update_code_svn", "Retrieves the latest code from SVN then sets up website."
recipe "app_iis::update_code_s3", "Retrieves the latest code from s3 then sets up website."
recipe "app_iis::start_default_website", "Starts the website named 'Default Web Site' if it is not already running"
recipe "app_iis::deploy_code_svn", "Retrieves code from SVN and sets up the website only if the code has not already been deployed"
recipe "app_iis::deploy_code_s3", "Retrieves code from s3 and sets up the website only if the code has not already been deployed"

attribute "app_iis/releases_dir",
  :display_name => "IIS Releases Dir",
  :description => "The full directory path where code will be deployed and updated",
  :recipes => ["app_iis::update_code_svn","app_iis::deploy_code_svn","app_iis::update_code_s3","app_iis::deploy_code_s3"],
  :required => "optional",
...
attribute "svn/repo_path",
  ...
  :recipes => ["app_iis::update_code_svn","app_iis::deploy_code_svn"],
  :required => "required"
...
attribute "aws/access_key_id",
  ...
  :recipes => ["app_iis::update_code_s3","app_iis::deploy_code_s3"],
  :required => "required"
